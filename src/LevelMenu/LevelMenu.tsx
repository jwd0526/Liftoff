    ...
    <div className="level-menu-container">
      <div className="panel-container" id="panel-img">
        <img
          className="panel-img"
          src={levelMenuPanel}
          alt="level-menu-panel"
        />
        <div className="panel-content">
          <h1 className="panel-header">Upgrades</h1>
          <div className="capacity-headers headers">
            <h2 className="capacity-header p1">
              <sup></sup>Fuel Capacity:{" "}
            </h2>
            <h2 className="capacity-header p2">
              {(state.capacity / Math.pow(10, capacityExponent)).toFixed(2)} x
              10<sup>{capacityExponent + 3}</sup> kg
            </h2>
          </div>
          <div className="allocate-box">
            <div className="level-up">
              <img
                className={`level-up-img ${state.points > 0 ? "" : "no-press"}`}
                src={levelUp}
                alt="level-up"
                onClick={() => dispatch({ type: "increaseCapacity" })}
              />
            </div>
            <p className="point-value">{state.capacityPoints}</p>
            <div className="level-down">
              <img
                className={`level-down-img ${state.capacityPoints > 0 ? "" : "no-press"}`}
                src={levelDown}
                alt="level-down"
                onClick={() => dispatch({ type: "decreaseCapacity" })}
              />
            </div>
          </div>
          ...
          <div className="power-headers headers">
            ...
            <h2 className="power-header p2">
              {(5 + state.power).toFixed(2)} MJ/L
            </h2>
          </div>
          <div className="allocate-box">
            <div className="level-up">
              <img
                ...
                src={levelUp}
                alt="level-up"
                onClick={() => dispatch({ type: "increasePower" })}
              />
            </div>
            <p className="point-value">{state.powerPoints}</p>
            <div className="level-down">
              <img
                className={`level-down-img ${state.powerPoints > 0 ? "" : "no-press"}`}
                src={levelDown}
                alt="level-down"
                onClick={() => dispatch({ type: "decreasePower" })}
              />
            </div>
          </div>
          ...
          <div className="allocate-box">
            <div className="level-up">
              <img
                ...
                src={levelUp}
                alt="level-up"
                onClick={() => dispatch({ type: "increaseForcefield" })}
              />
            </div>
            <p className="point-value">{state.forcefieldPoints}</p>
            <div className="level-down">
              <img
                className={`level-down-img ${state.forcefieldPoints > 0 ? "" : "no-press"}`}
                src={levelDown}
                alt="level-down"
                onClick={() => dispatch({ type: "decreaseForcefield" })}
              />
            </div>
          </div>
          ...
          <div className="buy-box">
            <button
              className={`buy-point ${state.coins > state.cost ? "" : "no-press"}`}
              id="buy-point"
              onClick={() => {
